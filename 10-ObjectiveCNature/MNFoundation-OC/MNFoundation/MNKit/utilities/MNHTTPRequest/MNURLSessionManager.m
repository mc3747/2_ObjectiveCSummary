...
- (void)configSessionSerializationWithRequest:(__kindof MNURLRequest *)request {
    ...
    _session.requestSerializer.headerFields = request.headerFields;
...
- (NSURLSessionUploadTask *)uploadTaskWithRequest:(__kindof MNURLUploadRequest *)request {
    ...
    [self configSessionSerializationWithRequest:request];
    uploadTask = [_session uploadTaskWithUrl:request.url parameter:request.parameter body:request.bodyCallback uploadProgress:request.progressCallback completion:^(NSURLResponse *response, id responseObject, NSError *error) {
        __strong typeof(self) self = weakself;
