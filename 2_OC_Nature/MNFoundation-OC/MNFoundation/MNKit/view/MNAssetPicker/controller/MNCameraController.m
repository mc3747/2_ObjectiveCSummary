...
@interface MNCameraController () <MNCapturingViewDelegate, MNCapturerDelegate, MNPlayerDelegate>
...
- (void)capturingViewDoneButtonClicked:(MNCapturingView *)capturingView {
    ...
    if (self.previewView.alpha) {
        ...
        // 判断时长是否符合限制要求
        NSTimeInterval duration = self.capturer.duration;
        ...
        if ([self.delegate respondsToSelector:@selector(cameraController:didFinishWithVideoOfPath:)]) {
            [self.delegate cameraController:self didFinishWithVideoOfPath:self.capturer.outputPath];
        }
    }
    // 回调内容
...
- (void)capturerDidEndWithError:(MNCapturer *)capturer {
    if (capturer.error.code == AVErrorApplicationIsNotAuthorized) {
        [[MNAlertView alertViewWithTitle:@"提示" message:capturer.error.localizedDescription handler:^(MNAlertView *alertView, NSInteger buttonIndex) {
            if ([self.delegate respondsToSelector:@selector(cameraControllerDidCancel:)]) {
                [self.delegate cameraControllerDidCancel:self];
            }
