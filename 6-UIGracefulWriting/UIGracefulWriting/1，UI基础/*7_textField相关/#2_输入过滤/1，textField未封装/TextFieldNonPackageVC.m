...
#import "TextFieldNonPackageVC.h"

static NSInteger CharacterCount = 8;

@interface TextFieldNonPackageVC ()<UITextFieldDelegate>
@property (strong, nonatomic)  UILabel        *totalCharacterLabel;
@property (strong, nonatomic)  UITextField    *nameField;
@end

@implementation TextFieldNonPackageVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.totalCharacterLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 100, 50, 30)];
    self.totalCharacterLabel.backgroundColor = [UIColor grayColor];
    self.totalCharacterLabel.text = @"8";
    
    self.nameField = [[UITextField alloc] initWithFrame:CGRectMake(0, 200, 200, 50)];
    self.nameField.backgroundColor = [UIColor grayColor];
    self.nameField.placeholder = @"输入内容";
    [self.nameField addTarget:self action:@selector(textFieldChanged:) forControlEvents:UIControlEventEditingChanged];
    self.nameField.delegate = self;
    
    [self.view addSubview:self.totalCharacterLabel];
