...
- (void)viewDidLoad {
    ...
    CGFloat height =   [self.textview.attributedText boundingRectWithSize:CGSizeMake(SCREEN_WIDTH, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin context:nil].size.height;
    
    self.textview.height = height;
    self.textview.contentSize = CGSizeMake(SCREEN_WIDTH, height);
...
{
    if (!_textview) {
        _textview =[[UITextView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 40)];
